...
// https://github.com/wangzhaode/mnn-yolo/blob/main/cpp/yolov8_demo.cpp
void test_yolovx_mnn(const ModelConfig& config, const cv::Mat& bgr, int nloops, int useGpu)
{
    ...
    const std::string modelFile = modelPrefix + ".mnn";
    ...
    sConfig.type = static_cast<MNNForwardType>(MNN_FORWARD_CPU);
    sConfig.numThread = 4;
    BackendConfig bConfig;
    bConfig.precision = static_cast<BackendConfig::PrecisionMode>(2);
    sConfig.backendConfig = &bConfig;
    std::shared_ptr<Executor::RuntimeManager> rtmgr 
        = std::shared_ptr<Executor::RuntimeManager>(Executor::RuntimeManager::createRuntimeManager(sConfig));
    if(rtmgr == nullptr) {
    ...
    std::shared_ptr<Module> net(Module::load(std::vector<std::string>{}, std::vector<std::string>{}, modelFile.c_str(), rtmgr));
    auto original_image = CV::imread(argv[2]);
    ...
    auto pads = _Const(static_cast<void*>(padvals.data()), {3, 2}, CV::NCHW, halide_type_of<int>());
    auto image = _Pad(original_image, pads, CV::CONSTANT);
    image = resize(image, Size(640, 640), 0, 0, CV::INTER_LINEAR, -1, {0., 0., 0.}, {1./255., 1./255., 1./255.});
    auto input = _Unsqueeze(image, {0});
    input = _Convert(input, CV::NC4HW4);
    auto outputs = net->onForward({input});
    auto output = _Convert(outputs[0], CV::NCHW);
    ...
    auto start = _Const(static_cast<void*>(startvals.data()), {2}, CV::NCHW, halide_type_of<int>());
    std::vector<int> sizevals { -1, -1 };
    auto size = _Const(static_cast<void*>(sizevals.data()), {2}, CV::NCHW, halide_type_of<int>());
    ...
    for (int i = 0; i < 100; i++) 
    {
        ...
        CV::rectangle(original_image, {x0, y0}, {x1, y1}, {0, 0, 255}, 2);
    }
