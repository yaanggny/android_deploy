...
int PicoDet::load(const char *param, const char *bin, int input_width,
                 int input_hight, bool useGPU, float score_threshold_ = 0.5,
                 float nms_threshold_ = 0.3) {
  this->Net = new ncnn::Net();
  this->Net->opt.num_threads = ncnn::get_big_cpu_count();
#if NCNN_VULKAN
...
int PicoDet::detect(cv::Mat image, std::vector<BoxInfo> &result_list,
                    bool has_postprocess, bool new_model) {
...
#endif
  ex.input(new_model?"in0":"image", input); // picodet
  ...
  if (has_postprocess) {
    ...
    std::vector<NonPostProcessHeadInfo> headInfos = new_model? this->non_postprocess_heads_info2: this->non_postprocess_heads_info;
    for (const auto &head_info : headInfos) {
      ncnn::Mat dis_pred;
      ncnn::Mat cls_pred;
