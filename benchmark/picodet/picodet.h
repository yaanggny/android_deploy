...
class PicoDet {
public:
  PicoDet(const char *modelPrefix, int input_width, int input_hight,
          bool useGPU, float score_threshold_, float nms_threshold_)
  {
    ...
    load(parampath, modelpath, input_width, input_hight, useGPU, score_threshold_, nms_threshold_);
  }

  PicoDet() {}

  ~PicoDet();
  int load(const char *param, const char *bin, int input_width, int input_hight,
          bool useGPU, float score_threshold_, float nms_threshold_);
  ...
  int detect(cv::Mat image, std::vector<BoxInfo> &result_list,
             bool has_postprocess, bool new_model=false);
...
private:
  ...
  std::vector<NonPostProcessHeadInfo> non_postprocess_heads_info2{
      // cls_pred|dis_pred|stride
      {"out0", "out4", 8},
      {"out1", "out5", 16},
      {"out2", "out6", 32},
