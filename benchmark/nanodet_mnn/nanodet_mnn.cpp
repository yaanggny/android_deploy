...
NanoDet::NanoDet(const std::string &mnn_path, int num_thread_)
{
...
int NanoDet::detect(cv::Mat &raw_image, std::vector<BoxInfo> &result_list, float score_threshold_, float nms_threshold_)
{
    if (raw_image.empty()) {
    ...
    MNN::Tensor *tensor_preds = NanoDet_interpreter->getSessionOutput(NanoDet_session, m_config.outputNode.c_str());
    ...
    std::cout << "detect " << result_list.size() << " objects" << std::endl;
...
{
    bool isPlus = m_config.modelname_noext.find("plus") != std::string::npos;
    float offset = (1-int(isPlus)) * 0.5; 

    float ct_x = (x + offset) * stride;
    float ct_y = (y + offset) * stride;
